...
data MachineCmd : (ty : Type) -> VendState -> (ty -> VendState) -> Type where
  InsertCoin : MachineCmd _ (pounds, chocs)     (\res => case res of
  ...
  (>>=) : MachineCmd a state1 state2_fn -> ((res : a) -> MachineCmd b (state2_fn res) state3) -> MachineCmd b state1 state3
  ...
  machineLoop =
        do Just x <- GetInput
               | Nothing => do Display "Invalid input"
                               machineLoop
           case x of
                COIN => do check <- InsertCoin
                           case check of
                                Inserted => do Display "Coin was accepted"
                                               machineLoop
                                Rejected => do Display "Coin was rejected"
                                               machineLoop
                VEND => vend
                CHANGE => do GetCoins
